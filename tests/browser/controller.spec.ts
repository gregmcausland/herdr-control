...
test("hands writable control between browser clients explicitly", async ({ browser }) => {
  ...
  await expect(first.locator(".terminal-header small")).toHaveText(new RegExp(`${paneId} · Control$`));
  ...
  await expect(second.locator(".terminal-header small")).toHaveText(new RegExp(`${paneId} · Observing$`));

  await second.getByRole("button", { name: "Control here" }).click();
  await expect(second.locator(".terminal-header small")).toHaveText(new RegExp(`${paneId} · Control$`));
