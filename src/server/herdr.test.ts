...
describe("Thread messages", () => {
  it("renames a thread pane and tab through imperative Herdr commands", async () => {
    const calls: unknown[] = [];
    const herdr = new HerdrAdapter("herdr", "/tmp/herdr.sock", async (_socket, method, params) => {
      calls.push({ method, params });
      return { result: { type: "workspace_info" } };
    });
    await herdr.renameThread("w1:p1", "w1:t1", "Improve voice input");
    expect(calls).toEqual([{ method: "pane.rename", params: { pane_id: "w1:p1", label: "Improve voice input" } },
      { method: "tab.rename", params: { tab_id: "w1:t1", label: "Improve voice input" } }]);
  });
